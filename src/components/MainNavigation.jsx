...
const MainNavigation = ({ scrollPosition, data, prefix = '' }) => {
  ...
  return ( 
    <nav style={{ boxShadow: colors.primaryColor, backgroundColor: colors.backgroundColor }} className={` fixed ${scrollPosition > 82 ? ' shadow-xl shadow-[#faa819]/10' : ' shadow-lg' } shadow-[#faa819]/30l shadow-lgk sha top-0 left-0 right-0 m-auto w-full max-w-[1920px] z-20 flex justify-between items-center px-8 md:px-12 lg:px-24 2xl:px-32 h-16 md:h-32 ease-in-out transition-all`}>
      ...
      <a href="/">
        <img className=' cursor-pointer h-12 xl:h-18 w-auto hover:scale-105 ease-in-out transition-all' src="/logo.png" height={'auto'}  alt="White Appletree logo" />
      </a>

      <ul className=' hidden xl:flex gap-8 text-base pr-4' style={{ color: primaryColor }}>
        {
          data.map(({ active, link, name }) => (
            <a href={`${prefix}${link}`}>
              <li key={name} className={` px-8 py-2 rounded-lg text-lg hover:scale-105 hover:shadow-gray-300 ${active ? 'bg-[#faa819] text-gray-50 hover:shadow-xl' : 'hover:bg-[#faa819] hover:text-gray-50 hover:shadow-xl'} ease-in-out transition-all`}>
                {name}
              </li>
            </a>
          ))
