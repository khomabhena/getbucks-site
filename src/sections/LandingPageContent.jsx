    ...
    <section className='mt-8 md:mt-24 flex flex-col items-center'>
      ...
      
      {
        data?.map(({ index, image, title, description }) => (
          ...
          <div className={` ${index % 2 == 0 ? 'flex-row-reverse' : 'flex-row'} flex items-center justify-between gap-24 mt-12`}>
            <section className=' flex flex-col basis-2/4 px-12'>
              <h2 className=' text-3xl font-semibold'>{title}</h2>
              <p className=' text-gray-700 mt-6'>{description}</p>
            </section>
            <div className=' basis-2/4 pl-12'>
