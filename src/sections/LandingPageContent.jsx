    ...
    <section className='mt-8 md:mt-24 flex flex-col items-center'>
      ...
      
      {
        data?.map(({ index, image, title, description }) => (
          ...
          <div key={index} className={` ${index % 2 == 0 ? 'xl:flex-row-reverse' : 'xl:flex-row'} bg-red-600k flex flex-col w-full items-center justify-between gap-12m mt-12`}>
            
            <section className={` mr-32l mt-12 md:mt-0 flex flex-col basis-1 md:basis-3/5 xl:px-12`}>
              <h2 className=' text-lg xl:text-3xl font-semibold'>{title}</h2>
              <p className=' text-sm xl:text-base  text-gray-700 mt-6'>{description}</p>
            </section>

            <div className=' bg-green-600k basis-1 md:basis-2/5 mt-12 xl:pl-12'>
